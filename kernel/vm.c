...
int dup_current_virt_memory(struct mm_struct *dstmm) {
	...
	for (unsigned long i = 0; i < srcmm->sz; i += PAGE_SIZE) {
		...
		//printf("Got to before memmove 1\n");
		memmove(kernel_va, (void*)i, PAGE_SIZE); /* TODO: replace this */
		//printf("Got to after memmove 1\n");
	}
	...
	for (unsigned long i = PGROUNDDOWN(regs->sp); i < USER_VA_END; i+=PAGE_SIZE) {
		...
		void *kernel_va = allocate_user_page_mm(dstmm, i, PTE_TO_PERM(*pte)); /* TODO: replace this */
		if(kernel_va == 0)
			goto no_mem; 
		// NB: "i" is a userva in the src address space. this assumes the src's userva is active
		V("kern va %lx i %x", kernel_va, i);
		//printf("Got to before memmove 2\n");
		memmove(kernel_va, (void*)i, PAGE_SIZE); /* TODO: replace this */
		//printf("Got to after memmove 2\n");
	}
...
void create_kern_pgtables(void) {
	...
	create_block_map_section(pmd2, DEVICE_LOW, 
		VA_START + DEVICE_LOW, VA_START + DEVICE_HIGH - SECTION_SIZE, MMU_DEVICE_FLAGS); 
}
